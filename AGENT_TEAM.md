...
Before creating any project, process, skill, service, deployment, cron job, or Kanban card, run canonical discovery across the project inventory, filesystem and Git repositories, active and archived Kanban tasks, installed skills, cron jobs, deployments, and session history. Reuse or enhance the canonical artifact when one exists; prefer contribute, absorb, or consolidate over duplicate creation. Record the discovery result and canonical path/URL in the new card when creation is still justified.
