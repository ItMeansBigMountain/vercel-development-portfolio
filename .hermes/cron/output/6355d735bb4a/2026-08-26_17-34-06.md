...
⚠️ Needs attention
Uploaded: **0 URLs**; no new source reached **5/5**
Queue: **1** protected rendered clip; upload blocked
Blocker: all 3 YouTube profiles return `invalid_grant`; discovery never started
📌 Reauthorize Classical Echos, Sosai Oyama, and A F via `youtube_auth_healthcheck.py --verbose`, return each full localhost callback URL, then rerun
