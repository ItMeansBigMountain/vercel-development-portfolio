...
- Route all social-video and content-publishing crons to the unified Content Creation Discord channel (currently channel ID `1524259261281206332`), not General. The lane now covers YouTube, TikTok, Instagram/Facebook Reels, Threads, LinkedIn, X, Pinterest, Reddit, Bluesky, Parrot AI assets, original AI content, regular content, publishing, analytics, and community management. General is reserved for the daily stand-up/orchestrator report; specialist cron output belongs in Content Creation.
...
⚠️ BLOCKED / PARTIAL — Viral Radar

- Queue: 1 rendered clip (Chris Williamson, 11MB) stuck — upload failed `invalid_grant` (token expired/revoked), last attempt 2026-08-25
- New discovered sources (4 Hamza manifests Aug 25): all `youtube_download_required=true`; no `SOURCES/` mp4 present → `blocked_source`; each has only 1 planned clip (need ≥5 per source)
- Auth fix needed: refresh Classical Echos OAuth (`youtube_auth_healthcheck.py --verbose`) + new URL for the failed token before replay

Next: repair token → retry queued clip + source-download the 4 Hamza videos → render 5 clips each → upload. Details in `logs/` and `UPLOAD_QUEUE/`.
