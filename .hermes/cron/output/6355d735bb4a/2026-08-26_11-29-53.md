...
⚠️ Needs attention
Uploaded: **0/5**; no public URLs.
Queue: **1** protected rendered clip with metadata.
Blocker: all 3 YouTube OAuth profiles returned `invalid_grant`; new sources were never seeded/rendered.
📌 Reauthorize Classical Echos, Trapiistan, and A F using the fresh healthcheck URLs, then rerun.
