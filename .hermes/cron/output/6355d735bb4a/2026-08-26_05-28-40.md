...
⚠️ Needs attention
Uploaded: **0** URLs; no new sources reached **5/5**
Queue: **1** rendered Chris Williamson clip preserved
Blocker: OAuth `invalid_grant` on Classical Echos, Sosai Oyama, and A F; discovery/rendering never started
📌 Reauthorize all 3 using the fresh healthcheck links, return each full localhost callback URL, then rerun queue-first processing
