...
⚠️ Needs attention
Uploaded URLs: none; new sources: none selected (auth-blocked before 5/5 planning).
Queue: **1** protected rendered clip; drain attempted first, upload failed.
Blocker: all 3 YouTube profiles return `invalid_grant`; watchlist seeding also failed.
Next: reauthorize Classical Echos, Sosai Oyama, and A F using the fresh OAuth links, then return all 3 full localhost callback URLs.
