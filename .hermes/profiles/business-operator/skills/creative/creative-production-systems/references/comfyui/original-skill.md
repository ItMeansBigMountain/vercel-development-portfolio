...
prerequisites:
  commands: ["python3"]
setup:
  help: "Run references/comfyui/hardware_check.py FIRST to decide local vs Comfy Cloud; then references/comfyui/comfyui_setup.sh auto-installs locally (or use Cloud API key for platform.comfy.org)."
metadata:
  hermes:
    tags:
      - comfyui
...
python3 references/comfyui/hardware_check.py
```
...
```bash
python3 references/comfyui/health_check.py
...
```bash
python3 references/comfyui/extract_schema.py workflow_api.json --summary-only
# → {"parameter_count": 12, "has_negative_prompt": true, "has_seed": true, ...}

python3 references/comfyui/extract_schema.py workflow_api.json
...
python3 references/comfyui/run_workflow.py \
...
python3 references/comfyui/run_batch.py \
  --workflow sdxl.json \
...
| "install ComfyUI" | comfy-cli | `bash references/comfyui/comfyui_setup.sh` |
...
```bash
python3 references/comfyui/hardware_check.py --json
# Optional: also probe `torch` for actual CUDA/MPS:
python3 references/comfyui/hardware_check.py --json --check-pytorch
```
...
```bash
bash references/comfyui/comfyui_setup.sh
# Or with overrides:
bash references/comfyui/comfyui_setup.sh --m-series --port=8190 --workspace=/data/comfy
```
...
python3 references/comfyui/check_deps.py my_workflow.json
...
python3 references/comfyui/fetch_logs.py --tail-queue --host https://cloud.comfy.org
```
...
Use `python3 references/comfyui/health_check.py` to run the whole list at once. Manual:
